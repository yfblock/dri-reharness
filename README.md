...
- 多源规模：C67X00（4 C）、ASPEED vHub（5 C）与 DWC2 dual-role（10 C），合计 19 TU / 27,447 LoC；harness/bare-metal 3/3 编译，Linux 1/3，失败案例保留明确 blocker。
...
- 测试套件：78 tests。
- 可靠性审计：每个 source site 与 RIS op 均带稳定证据；机器报告给出 scoped strict 8/19，并明确 `whole_program_complete=false`。
...
RIS leaf op 还包含 `op_id`、source evidence、reliability、address/value/path precision 和 access domain。已识别 MMIO/regmap API、直接 volatile 解引用与 inline asm 都进入 access accounting；无法 lowering 的访问不会静默消失。结构化路径由 Z3 检查可满足性与 switch 互斥性；规范、静态有界的 `for` 循环可被证明并生成，其余循环保持 conservative。简单参数型 early exit 会转成 continuation guard，goto 与影响寄存器循环的 break/continue 由 control accounting 显式阻塞。
