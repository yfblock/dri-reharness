...
- 三个确定性后端：19/19 均可编译。
- 严格语义 readiness：harness 6/19、bare-metal 6/19、Linux 6/19；可编译不等于语义完整。
- 多源规模：C67X00（4 C）、ASPEED vHub（5 C）与 DWC2 dual-role（10 C），合计 19 TU / 27,447 LoC；三后端均可编译。
- 跨 TU 质量：974 条内部调用边，其中 223 条跨 TU 边全部解析；568 条调用边传播了 MMIO 摘要。
- 原始 MMIO 对照：907 个源码 primitive、927 个 direct AST 操作、3,469 个传播后 RIS MMIO 操作；DWC2 恢复 46 个命名寄存器。
- 测试套件：55 tests。
...
Linux lowering 会区分 callback table 的具体实例。GPIO 动态 `gpio_irq_chip.init_hw` 绑定会按字段语义归类；clock provider 会保留多套 `clk_ops`、纯标量 rate 算术、源码内 helper、父时钟/provider 注册以及对应 OF 变体。只有经过保守 source-private 重绑定且真实 Kbuild 通过的 callback 才可消除 unsupported marker。
