...
- 测试套件：92 tests；测试入口先执行冻结 holdout/specialization guard。
...
- 零样本矩阵：12/12 holdout 使用可审计 exact compile context，12/12 pipeline 与三后端编译完成，3/12 三后端 strict-ready。机器聚类识别出的首个公共语义 blocker 是 7/12 的 `no_register_access`；这表示框架/库间接寄存器访问尚未进入 RIS，不能把空生成物的编译成功写成泛化成功。
