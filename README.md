...
- 三个确定性后端：19/19 均可编译。
- 严格语义 readiness：harness 2/19、bare-metal 2/19、Linux 7/19。GPIO generic-library callbacks 现在进入 RIS/Linux lowering，但通用后端尚无 callback execution oracle，因此不再沿用旧的 6/19 假阳性。
...
- 测试套件：100 tests；测试入口先执行冻结 holdout/specialization guard。
...
- QEMU：edu 通过值级 oracle；gpio-ftgpio010 通过结构化 Formal RIS、精确函数边界和真实 gpiolib exerciser 的 probe/callback MMIO oracle（6/6 模块、7/7 调用、16/16 ops、7/7 寄存器偏移）。
...
- Subsystem summaries：冻结矩阵中原先 7 个 `no_register_access` 已降为 0。GPIO 从 typed `gpio_generic_chip_config` 合成 callback；SDHCI accessor/ops table 与 virtio config/virtqueue 分域记录。12/12 三后端仍编译；strict-ready 为 harness 3/12、bare-metal 3/12、Linux 5/12，首个公共根因转为 5/12 的 generic-backend callback oracle 缺口。

当前冻结实验结果来自 experiments/results/matrix.json：

| 驱动数 | Ops | Symbolic | Fixed | Computed | RMW | Conditions | Registers |
|---:|---:|---:|---:|---:|---:|---:|---:|
| 19 | 469 | 356 | 74 | 25 | 88 | 117 | 157 |
