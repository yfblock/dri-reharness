...
环境中没有 `pytest` executable。新增测试先通过直接调用测试函数验证，最终仍由仓库自带 standalone runner 执行，C10 后结果为 92/92。项目不应把外部 pytest CLI 当作唯一 CI 入口。

## 5. C10 之后的下一步

1. 以 7/12 的 `no_register_access` 聚类为首要机制目标，区分 subsystem library access、regmap/opaque domain 和真正无寄存器驱动；
2. 禁止新增 basename/Kconfig 常量特例，所有扩展必须由 callback/resource/type 证据触发；
