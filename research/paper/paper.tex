...
We compare \reharness against a naive regex scanner that we implement as a baseline; it matches MMIO call sites (\texttt{readl}, \texttt{writel}, etc.) directly in the source text, in the style of early text-level kernel source audits~\cite{chou2001empirical}. The comparison focuses on the four failure patterns identified in Section~\ref{sec:motivation}.
