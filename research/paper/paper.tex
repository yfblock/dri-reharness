...
\node[block, right=of dspec] (gen) {Code Generation\\\texttt{src/generator/common.py}, \texttt{src/generator/harness.py}, \texttt{src/generator/baremetal.py}, \texttt{src/generator/linux.py}};
\node[artifact, right=of gen] (code) {Generated C Code\\Linux / Bare-metal / Harness};
...
\draw[arrow] (gen) -- (code) node[midway, above] {emitted deterministically};

\node[block, below=1.4cm of gen] (llm) {LLM Repair Loop\\\texttt{src/synthesis.py}};
\draw[arrow, dashed] (code.south) -- ++(0,-0.4) -| (llm);
\draw[arrow, dashed] (llm) -| (gen.south);

\end{tikzpicture}%
}
\caption{Implementation architecture of \reharness, showing repository components and artifact flow from C source through formal RIS and backend code generation.}
\label{fig:impl_arch}
\end{figure}

\textbf{Stage 1: RIS Extraction.} The extraction stage is implemented in \texttt{src/extractor/tu.py}, \texttt{src/extractor/macros.py}, and \texttt{src/extractor/ast\_model.py}. It parses C source with libclang, preserves preprocessing context, and extracts a macro table of \texttt{\#define} offsets. Target functions and callback initializer bindings are discovered from the AST. The extractor then computes a libclang-backed call graph and performs flow-sensitive dataflow analysis with guarded predicate stacking. The output is a formal \rislang specification recording every register read, write, and RMW operation with symbolic register names, branch guards, and width annotations.
...
\textbf{Stage 3: Code Generation.} Code generation is implemented in \texttt{src/generator/common.py}, \texttt{src/generator/harness.py}, \texttt{src/generator/baremetal.py}, \texttt{src/generator/linux.py}, and \texttt{src/generator/subsystem\_runner.py}. It consumes the formal specs and backend-specific bindings to emit deterministic C for three targets: a userspace harness with fake MMIO memory and trace logging, portable bare-metal C, and Linux kernel modules with platform, PCI, GPIO/IRQ, and clock-provider glue. When a source expression cannot be safely lowered or a lifecycle is unknown, the generator emits an explicit unsupported marker instead of approximating semantics.

\textbf{Stage 4: LLM-Assisted Synthesis.} The optional LLM repair loop is implemented in \texttt{src/synthesis.py}. When deterministic generation cannot fully reconstruct source-private state or subsystem-specific lifecycle semantics, \reharness assembles a structured bundle of \rislang, \texttt{DeviceSpec}, \texttt{BindSpec}, source facts, and a scaffolded candidate. It then uses compile, static, and trace verification feedback to prompt the LLM for repairs until the candidate passes the checks or the iteration budget expires.
...
When deterministic generation is insufficient---for example, when the Linux backend cannot fully reconstruct the probe/resource lifecycle or subsystem-specific registration glue---\reharness enters a closed-loop LLM-assisted synthesis mode (Figure~\ref{fig:impl_arch}, dashed path).
...
\textbf{Linux subsystem coverage.} \reharness recognizes GPIO and IRQ tables (\texttt{gpio\_chip}, \texttt{irq\_chip}, and \texttt{gpio\_irq\_chip}); bus drivers (\texttt{platform\_driver}, \texttt{pci\_driver}, and \texttt{amba\_driver}); virtio, clock, power-management, file-operation, and USB endpoint/gadget/HCD tables. USB callback signatures and common private fields are modeled, but endpoint allocation, gadget registration, and host-controller lifecycle reconstruction remain explicit unsupported bindings. Other subsystems, including I2C and SPI, require additional callback and lifecycle models.
