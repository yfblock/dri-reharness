...
\node[block, right=of src] (parse) {Parse + Macro Extraction\\\texttt{src/ast\_analyzer/}, \texttt{src/extractor/macros.py}};
\node[block, right=of parse] (extract) {RIS Extraction + Inlining\\\texttt{src/extractor/dataflow/}, \texttt{src/extractor/call\_graph/}, \texttt{src/extractor/wrappers.py}};
...
\node[block, right=of ris] (infer) {Semantic Inference\\\texttt{src/extractor/spec\_infer/}, \texttt{src/extractor/spec.py}};
\node[artifact, right=of infer] (dspec) {Formal Specs\\.dspec/.bind/.facts};
\node[block, right=of dspec] (gen) {Backend Plugins + LLM Bridge\\\texttt{src/backends/registry.py}, \texttt{src/backends/llm\_bridge.py}, \texttt{src/backends/\{harness,baremetal,linux,rust\_baremetal\}/}};
