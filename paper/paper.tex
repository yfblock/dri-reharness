...
A frozen 12-driver zero-shot holdout provides a separate generalization check. All three generated backends compile for 12/12 cases and are strictly ready for 7/12. Sequential \texttt{gpio-mmio} callbacks are checked against an independent source model that includes \texttt{sdata}/\texttt{sdir} shadow state and detects six semantic mutations. NPCM SDHCI is admitted only by a source contract that excludes private accessor overrides; Dove and HLWD remain rejected. The Altera IRQ loop is lowered only as a masked W1C drain: the pending and mask registers are reread after acknowledgement, and the one-body-iteration proof explicitly assumes that no new pending bits arrive during the drain. These contracts establish register-interaction behavior, not complete GPIO/IRQ or SDHCI lifecycle equivalence.
