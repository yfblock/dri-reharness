...
\node[block, right=of src] (parse) {Parse + Macro Extraction\\\texttt{extractor/tu.py}, \texttt{extractor/macros.py}, \texttt{extractor/ast\_model.py}};
\node[block, right=of parse] (extract) {RIS Extraction + Inlining\\\texttt{extractor/call\_graph.py}, \texttt{extractor/dataflow.py}, \texttt{extractor/wrappers.py}, \texttt{extractor/taint.py}};
\node[artifact, right=of extract] (ris) {Formal RIS\\.ris};
\node[block, right=of ris] (infer) {Semantic Inference\\\texttt{extractor/spec\_infer.py}, \texttt{extractor/spec.py}};
\node[artifact, right=of infer] (dspec) {Formal Specs\\.dspec/.bind/.facts};
\node[block, right=of dspec] (gen) {Code Generation\\\texttt{generator/common.py}, \texttt{generator/harness.py}, \texttt{generator/baremetal.py}, \texttt{generator/linux.py}};
...
\node[block, below=1.4cm of gen] (llm) {LLM Repair Loop\\\texttt{synthesis.py}};
