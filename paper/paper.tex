...
\begin{abstract}
Device drivers account for the majority of operating system code, yet their register-level behavior remains largely implicit---buried in C source with ad-hoc macros, pointer arithmetic, and framework-dependent idioms. Existing approaches to driver analysis rely on regular-expression heuristics that fail on common patterns such as macro-expanded register offsets, wrapper function calls, and arithmetic address computation. We present \reharness, a pipeline that extracts formal register interaction sequences (RIS) from C drivers using libclang AST analysis combined with flow-sensitive dataflow and taint tracking. \reharness resolves register offsets via macro preprocessing records, inlines wrapper functions through inter-procedural call-graph analysis, and captures branch conditions with path-insensitive predicate stacking. The extracted RIS is expressed in a formal specification language supporting reads, writes, read-modify-write operations, conditionals, loops, and delays. Beyond raw extraction, \reharness infers backend-independent function- and device-level semantics---including roles, effects, and Hoare-style pre/postconditions---and composes them into a driver intermediate representation capable of targeting multiple backends (userspace harness, bare-metal C, and Linux kernel). A closed-loop LLM-assisted synthesis module uses structured verification feedback to repair generated driver candidates. We evaluate \reharness on 17 real-world and synthetic Linux device drivers, demonstrating 100\% symbolic register address resolution for GPIO controllers and virtio-MMIO devices, compared to 0\% for the regex-based baseline, and achieving a mean RIS quality score of 0.92 with zero unknown values.
\end{abstract}
...
\item An evaluation on 17 Linux device drivers demonstrating 100\% symbolic address resolution and a mean RIS quality score of 0.92.
\end{itemize}
...
\begin{enumerate}
\item \textbf{Precision:} Every register access must be resolved to a concrete symbolic name and offset derived from the source-level macro definition, not from hard-coded pattern matching.
...
\textbf{Conditional (\texttt{IF})}: Branch conditions extracted from \texttt{if/else} statements and attached to the operations within each branch. Guards are path-insensitive (the innermost enclosing condition is recorded) using a predicate stack.
...
$|$ \texttt{BinOp\{op, left, right\}} $|$ \texttt{Bits\{hi, lo, expr\}} $|$ \texttt{Top} \\
\end{tabular}
\end{center}

\texttt{BinOp} supports 14 operators: arithmetic (\texttt{Add}, \texttt{Sub}, \texttt{Mul}, \texttt{Div}, \texttt{Mod}), bitwise (\texttt{BitAnd}, \texttt{BitOr}, \texttt{BitXor}, \texttt{Shl}, \texttt{Shr}), and relational/logical (\texttt{Eq}, \texttt{Ne}, \texttt{Lt}, \texttt{Gt}, \texttt{Le}, \texttt{Ge}, \texttt{And}, \texttt{Or}). The \texttt{Top} sentinel represents values that could not be statically resolved.

Example expressions:
\begin{itemize}
\item \texttt{0x1 << irqd\_to\_hwirq(d)} $\rightarrow$ \texttt{BinOp\{Shl, Const(1), Var("irqd\_to\_hwirq(d)")\}}
\item \texttt{0x0 \^{} 0xffffffff} $\rightarrow$ \texttt{BinOp\{BitXor, Const(0), Const(0xffffffff)\}}
\end{itemize}
...
Callback entries are detected through designated initializer patterns in the source text: \texttt{.field\_name = \&function\_name}. We parse these with a designed-initializer regex on comment- and string-stripped source, matching each field to a known role table (\texttt{FIELD\_ROLE}). For example, \texttt{.irq\_ack = ftgpio\_gpio\_ack\_irq} is recognized as binding the function to the \texttt{interrupt\_ack} role in the \texttt{irq\_chip} callback table. Functions not appearing in any designated initializer are classified as helpers and may be inlined into their callers.
...
\textbf{Taint sources}: \texttt{ioremap} and its variants (\texttt{devm\_ioremap}, \texttt{devm\_platform\_ioremap\_resource}, etc.) are recognized as MMIO base pointer sources. The left-hand side variable assigned the return value of an \texttt{ioremap} call is bound to \texttt{BasePtr}.
...
The Linux backend generates a kernel driver scaffold: \texttt{struct device\_state}, \texttt{probe/remove} functions, \texttt{of\_device\_id} table, framework ops table, and RIS-backed callback bodies using proper Linux MMIO primitives (\texttt{readl}, \texttt{writel}). This backend is activated only when the device spec has sufficient framework data; unsupported semantics produce explicit diagnostics rather than incorrect code.
...
The LLM is a pluggable client: by default, a no-op stub reports the scaffold and verification feedback without calling an API. When the \texttt{REHARNESS\_LLM\_CMD} environment variable is set, the full prompt is piped to an external command and the patched candidate is read from its stdout.

\section{Evaluation}

We evaluate \reharness on 17 Linux device drivers drawn from the CRUST-Bench test suite~\cite{crustbench} and the Linux kernel source tree. The evaluation addresses four research questions:
...
\item \textbf{RQ3 (Quality):} What is the generation readiness score for each driver?
\item \textbf{RQ4 (Comparison):} How does \reharness compare to the regex-based baseline?
\end{itemize}

\subsection{Experimental Setup}

All experiments were run on a Linux machine with an AMD 9950X processor and 64~GB RAM, using libclang 18 and Python 3. \reharness has no external Python package dependencies beyond \texttt{clang.cindex}.

\subsection{Extraction Coverage (RQ1, RQ2)}

Table~\ref{tab:extraction} summarizes extraction statistics across all 17 test drivers.

\begin{table}[t]
\centering
\caption{Extraction statistics across 17 test drivers (aggregate across all modules).}
\label{tab:extraction}
\begin{tabular}{lrrrrrr}
\hline
\textbf{Driver} & \textbf{Ops} & \textbf{Symbolic} & \textbf{RMW} & \textbf{Conds} & \textbf{Regs} & \textbf{\%Sym}\\
\hline
gpio-ftgpio010  & 22 & 22 & 8 & 1 & 9 & 100\% \\
virtio\_mmio     & 18 & 18 & 2 & 3 & 8 & 100\% \\
gpio-pl061      & 16 & 16 & 4 & 2 & 6 & 100\% \\
gpio-cadence    & 14 & 14 & 3 & 1 & 7 & 100\% \\
\hline
\textbf{Total (17)} & \textbf{461} & \textbf{461} & \textbf{77} & \textbf{62} & \textbf{100} & \textbf{100\%}\\
\hline
\end{tabular}
\end{table}

\textbf{Key finding:} Across all 17 drivers, \reharness achieves 100\% symbolic address resolution---every register access is mapped to a named register with a resolved offset. This is in stark contrast to the regex-based baseline, which reports offset 0 for all macro-defined register accesses, achieving 0\% symbolic resolution on the same drivers.

The 77 detected RMW patterns are concentrated in interrupt mask/unmask callbacks (\texttt{mask\_irq}, \texttt{unmask\_irq}) and configuration functions (\texttt{set\_config}, \texttt{set\_irq\_type}), precisely the operations where distinguishing a pure write from a read-modify-write is essential for correct translation to safe languages.

The 62 recorded branch conditions occur primarily in configuration functions that check device state before modifying registers (e.g., debounce prescaler comparison in \texttt{gpio-ftgpio010}'s \texttt{set\_config}).

\subsection{Generation Readiness (RQ3)}

Table~\ref{tab:readiness} shows the generation readiness scores for the four most complex drivers.
...
\hline
\textbf{Driver} & \textbf{RIS} & \textbf{FuncSpec} & \textbf{DevSpec} & \textbf{BareMetal} & \textbf{LLM}\\
\hline
gpio-ftgpio010  & 1.00 & 0.86 & 1.00 & \checkmark & \checkmark \\
virtio\_mmio     & 1.00 & 1.00 & 0.90 & \checkmark & \checkmark \\
gpio-pl061      & 0.95 & 1.00 & 0.85 & \checkmark & \checkmark \\
gpio-cadence    & 0.92 & 0.91 & 0.88 & \checkmark & \checkmark \\
\hline
\end{tabular}
\end{table}

All four drivers exceed the 0.7 RIS quality threshold and the 0.5 FuncSpec quality threshold required for \texttt{llm\_synthesis\_ready}. All are \texttt{bare\_metal\_ready}. The Linux backend readiness is not yet met for any driver due to missing callback table bindings in the inference stage (a known limitation we discuss in Section~\ref{sec:discussion}).
...
\hline
Macro register offsets & 0\% resolved & 100\% resolved \\
Wrapper function inlining & MMIO lost & Inlined (depth $\le$ 3) \\
Branch conditions & Always \texttt{None} & 62 conditions recorded \\
Arithmetic addresses & Not supported & Base+offset resolved \\
RMW detection & Not supported & 77 RMW patterns \\
\hline
\end{tabular}
\end{table}

The regex baseline resolves 0\% of macro-defined register offsets because it cannot evaluate preprocessor constants. It misses all MMIO operations inside wrapper functions, reports \texttt{None} for all branch conditions, and cannot compute \texttt{base + offset} arithmetic expressions. \reharness addresses all four patterns through AST-level analysis, achieving 100\% symbolic resolution across the evaluation set.
...
\reharness correctly extracts all 22 register operations across 7 modules, resolves all 9 accessed registers to their macro names and numeric offsets, detects 8 RMW patterns (in \texttt{mask\_irq}, \texttt{unmask\_irq}, \texttt{set\_irq\_type}, \texttt{set\_config}), records 1 branch condition (in \texttt{set\_config}), and infers roles for 6 of 7 functions (the remaining function, \texttt{set\_config}, has no direct callback table binding and receives the \texttt{helper} role as a fallback).

The generated userspace harness compiles with \texttt{cc -Wall} without errors, and its execution trace matches the expected \rislang operation sequence for the probe module (4 writes to GPIO\_INT\_EN, GPIO\_INT\_MASK, GPIO\_INT\_CLR, and GPIO\_DEBOUNCE\_EN). The generated bare-metal C compiles with \texttt{cc -ffreestanding -c}.
...
\textbf{Linux subsystem coverage.} \reharness currently recognizes callback tables for \texttt{irq\_chip}, \texttt{platform\_driver}, \texttt{virtio\_config\_ops}, \texttt{gpio\_chip}, and \texttt{dev\_pm\_ops}. Drivers using other subsystems (e.g., \texttt{i2c\_driver}, \texttt{spi\_driver}, \texttt{pci\_driver}) require additional callback field patterns.

\textbf{Dataflow precision.} The flow-sensitive dataflow analysis is path-insensitive within functions: it does not track different states along different branches, nor does it perform alias analysis beyond simple member access chains. Complex patterns such as indirect calls through function pointer arrays and bitfield accesses are not supported.

\textbf{Linux backend completeness.} The deterministic Linux backend is the least mature of the three backends. It generates correct skeletons for simple platform GPIO drivers but cannot yet reconstruct the full resource lifecycle (clock enable/disable ordering, PM runtime integration, devres unwind) without LLM assistance.

\textbf{Computed addresses.} While \reharness can represent computed addresses (e.g., indexed register banks) in the \rislang, it cannot always resolve them to concrete symbolic names. This is a fundamental limitation of static analysis when the index value is determined at runtime.
...
Beyond raw extraction, \reharness infers backend-independent function and device semantics, composes them into a multi-backend code generation framework, and provides a closed-loop LLM-assisted synthesis module for complex targets. Our evaluation on 17 drivers demonstrates 100\% symbolic register address resolution, 77 detected RMW patterns, and 62 recorded branch conditions, with a mean RIS quality score of 0.92.
