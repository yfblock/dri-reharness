...
Table~\ref{tab:multisource} covers \MultiSourceDrivers{} modules, \MultiSourceTUs{} translation units, \MultiSourceLines{} source lines, and \MultiSourceFunctions{} functions, yielding \MultiSourceOps{} operations, \MultiSourceRMW{} RMW patterns, and \MultiSourceRegisters{} mapped registers. Of \MultiSourceCallEdges{} internal call edges, all \MultiSourceCrossTUEdges{} cross-TU edges are resolved; \MultiSourcePropagatedEdges{} edges carry MMIO summaries. The source comparison finds \MultiSourceSourceMMIO{} lexical MMIO primitives and \MultiSourceRISMMIO{} emitted RIS MMIO operations after propagation. C67X00 demonstrates propagation through its low-level HPI implementation, ASPEED vHub covers endpoint/hub paths, and the ten-file DWC2 case exercises host, gadget, dual-role, endpoint, and DMA code. Harness and bare-metal compile for all three modules, whereas generated Linux compiles only for DWC2. ASPEED vHub and C67X00 retain explicit endpoint/HCD lifecycle, dynamic-address, path, and control-flow blockers. This regression from a purely build-oriented claim is intentional: the new accounting rejects outputs that previously compiled only after silently dropping or normalizing unsupported semantics.
...
Harness, bare-metal, and Linux outputs compile for \EvalDrivers{}/\EvalDrivers{} drivers. In contrast, strict readiness is \HarnessReadyCount{}/\EvalDrivers{} for harness, \BaremetalReadyCount{}/\EvalDrivers{} for bare-metal, and \LinuxReadyCount{}/\EvalDrivers{} for Linux; \LLMReadyCount{}/\EvalDrivers{} have sufficient structured context for assisted synthesis. Highbank illustrates why the backend counts differ: its source-aware Linux clock lowering preserves the polling and arithmetic callbacks, while generic harness/bare-metal lowering continues to reject the unbounded polling loops. Unknown transforms, unsafe computed addresses, target-source clang errors, and normalized source-private state block readiness even when code can be compiled. Lowerable computed expressions, such as PL061's banked GPIO address and Sodaville's two type-register banks, are generated exactly and do not block readiness. For Sodaville, readiness additionally requires the recovered Intel PCI ID, 12-line GPIO configuration, dat/set/dirout operations, and mask/unmask/EOI fasteoi lifecycle; these properties are asserted and the generated module passes the pinned Kbuild. The mean RIS quality is \EvalMeanRIS{}.

We additionally publish \path{experiments/results/reliability.json}. Each recognized source access receives a stable site identifier, every RIS leaf carries source evidence and precision labels, and path predicates are checked for satisfiability and switch exclusivity with Z3. Direct volatile dereferences, inline assembly, unsupported register domains, and unmodeled control transfers cannot disappear silently. The report explicitly scopes its strict verdict to recognized register-access and structured-control patterns and records \texttt{whole\_program\_complete=false}; SVF remains per-translation-unit rather than linked whole-program analysis.
...
\textbf{Dataflow precision.} The flow-sensitive analysis retains branch predicates and merges common switch/if RMW patterns into conditional expressions. It also preserves simple parameter-only early exits and proves canonical statically bounded \texttt{for} loops, while goto and register-affecting loop transfers remain explicit control-accounting blockers. It still does not maintain a general CFG fixpoint or a fully separate abstract store for arbitrary paths. Complex joins, recursive state, and unmodeled aggregate state can therefore exceed the native domain. SVF alias analysis is available as an opt-in mode (\texttt{auto} or \texttt{required}) because its external toolchain and analysis cost would otherwise weaken deterministic reproduction; the current SVF execution is per translation unit, not linked whole-program bitcode, and the default evaluation uses alias mode \texttt{off}.
