...
Table~\ref{tab:multisource} covers \MultiSourceDrivers{} modules, \MultiSourceTUs{} translation units, \MultiSourceLines{} source lines, and \MultiSourceFunctions{} functions, yielding \MultiSourceOps{} operations, \MultiSourceRMW{} RMW patterns, and \MultiSourceRegisters{} mapped registers. Of \MultiSourceCallEdges{} internal call edges, all \MultiSourceCrossTUEdges{} cross-TU edges are resolved; \MultiSourcePropagatedEdges{} edges carry MMIO summaries. The source comparison finds \MultiSourceSourceMMIO{} lexical MMIO primitives and \MultiSourceRISMMIO{} emitted RIS MMIO operations after propagation. C67X00 demonstrates propagation through its low-level HPI implementation, ASPEED vHub covers endpoint/hub paths, and the ten-file DWC2 case exercises host, gadget, dual-role, endpoint, and DMA code. All three generated backends compile for every module. Strict readiness remains conservative because USB endpoint/gadget/HCD lifecycle registration, some source-private objects, DWC2 clang diagnostics, and unsafe dynamic addresses remain explicit blockers.
