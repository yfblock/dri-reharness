...
/* ---------------------------------------------------------------------- */
/* Harness MMIO environment: register-file model (host build executes     */
/* without a device; lane-addressed within 32-bit words).                 */
/* ---------------------------------------------------------------------- */
static uint32_t rh_regfile[1024];

static inline uint32_t rh_lane_shift(uintptr_t addr, unsigned width)
{
	return 8u * (unsigned)((addr & 3u) & ~((4u - width / 8u) & 3u));
}
...
{
	return (uint8_t)(rh_regfile[(addr >> 2) & 0x3ffu]
			 >> (8u * (addr & 3u)));
}

static inline uint16_t mmio_read16(uintptr_t addr)
{
	unsigned sh = 8u * (unsigned)(addr & 2u);
	return (uint16_t)((rh_regfile[(addr >> 2) & 0x3ffu] >> sh) & 0xffffu);
}

static inline uint32_t mmio_read32(uintptr_t addr)
{
	return rh_regfile[(addr >> 2) & 0x3ffu];
}

static inline uint16_t mmio_read16be(uintptr_t addr)
{
	uint16_t v = (uint16_t)((rh_regfile[(addr >> 2) & 0x3ffu]
	                         >> (8u * (unsigned)(addr & 2u))) & 0xffffu);
	return rh_bswap16(v);
}

static inline uint32_t mmio_read32be(uintptr_t addr)
{
	return rh_bswap32(rh_regfile[(addr >> 2) & 0x3ffu]);
}

static inline void mmio_write8(uint8_t value, uintptr_t addr)
{
	uint32_t *w = &rh_regfile[(addr >> 2) & 0x3ffu];
	unsigned sh = 8u * (unsigned)(addr & 3u);
...
static inline void mmio_write16(uint16_t value, uintptr_t addr)
{
	uint32_t *w = &rh_regfile[(addr >> 2) & 0x3ffu];
	unsigned sh = 8u * (unsigned)(addr & 2u);
	*w = (*w & ~(0xffffu << sh)) | ((uint32_t)value << sh);
}

static inline void mmio_write32(uint32_t value, uintptr_t addr)
{
	rh_regfile[(addr >> 2) & 0x3ffu] = value;
}
...
/* ---------------------------------------------------------------------- */
/* Oracle main (harness backend only): drive every evidence entry point   */
/* on a static zeroed device instance so the binary runs host-side.       */
/* ---------------------------------------------------------------------- */
int main(void)
{
	static struct dw_apb_ssi_priv ctlr_storage;
	struct dw_apb_ssi_priv *ctlr = &ctlr_storage;
	uint32_t rc = 0u;

	/* point the register window at the harness register file so every
	 * raw volatile dereference in an anchor body lands in backed memory */
	ctlr_storage.regs = (uintptr_t)rh_regfile;

	rc |= dw_spi_setup(ctlr);
	rc |= dw_spi_transfer_one(ctlr, ctlr, ctlr);
	rc |= dw_spi_irq(7u, ctlr);
	rc |= dw_spi_exec_mem_op(ctlr, ctlr);
	rc |= dw_spi_target_abort(ctlr);
	dw_spi_handle_err(ctlr, ctlr);
	dw_spi_cleanup(ctlr);
