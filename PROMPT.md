...
- tests：100 passed；`./run.sh test` 首先执行 zero-shot specialization guard。
...
- zero-shot matrix：12/12 exact context、pipeline 与三后端编译；原 7 个 `no_register_access` 已全部获得 subsystem summary。strict-ready 为 H=3/12、B=3/12、Linux=5/12；首个公共根因是 5/12 的 generic-backend callback execution oracle 缺口。
...
~~~text
19 drivers, 469 ops
356 symbolic, 74 fixed, 25 computed
...
- Linux 19/19 Kbuild，严格 ready 为 7/19；Harness/Bare 为 2/19。四个既有 generic-GPIO 案例不再因忽略框架 callback 而计入通用后端 ready。
...
- `drivers/holdout/zero-shot-v1-contexts.json` 固定 7 个 context profile。9 个案例使用对应架构 defconfig；3 个使用 pinned x86 build，其中 `gpio-ge` 明确标记为 non-native，因为 clang-18 拒绝原生 85xx `-mcpu=8540`。不得删掉该标记或把它写成 native context。
