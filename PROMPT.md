...
- tests：92 passed；`./run.sh test` 首先执行 zero-shot specialization guard。
...
- zero-shot matrix：12/12 exact context、12/12 pipeline、12/12 三后端编译、3/12 三后端 strict-ready；首个覆盖至少三个驱动的公共根因是 7/12 的 `no_register_access`。`strict_complete` 的 access accounting 若识别集合为空，不能代替“确实识别了硬件访问”。
