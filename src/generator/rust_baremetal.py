...
def expr_to_rust(e):
    ...
    if "Const" in e:
        return "0x%x" % e["Const"]
    if "Var" in e:
        return rust_ident(e["Var"])
    if "Top" in e:
        return "0 /* TODO: unknown */"
    if "BinOp" in e:
        b = e["BinOp"]
        op = rust_binop(b["op"])
        left = expr_to_rust(b["left"])
        right = expr_to_rust(b["right"])
    ...
    if "Ite" in e:
        ...
        return "(if %s { %s } else { %s })" % (
            expr_to_rust(i["guard"]),
            expr_to_rust(i["then"]),
            expr_to_rust(i["else"]))
    if "Bits" in e:
        b = e["Bits"]
        inner = expr_to_rust(b["expr"])
        width = b["hi"] - b["lo"] + 1
...
def rust_width_type(width):
    return {"B1": "u8", "B2": "u16", "B4": "u32", "B8": "u64"}.get(width, "u32")


def addr_to_rust(addr, base_expr, register_consts, state_expr="dev"):
    if "Symbolic" in addr:
    ...
    if "Computed" in addr:
        return expr_to_rust(addr["Computed"])
    return base_expr


def rust_mmio_primitive(operation, body):
...
def ops_to_rust(ops, bind, base_expr, register_consts, indent=1,
    ...
    for op in ops:
        ...
        if leaf is not None and leaf.get("reliability") == "Unsupported":
            ...
            continue
        if "Cond" in op:
            guard = expr_to_rust(op["Cond"]["guard"])
            out.append("%sif %s {" % (pad, guard))
            out.append(ops_to_rust(op["Cond"]["then_ops"], bind, base_expr,
                                    register_consts, indent + 1, state_expr,
                                    _anchor_ids, _lowering_recipes))
            if op["Cond"].get("else_ops"):
                out.append("%s} else {" % pad)
                out.append(ops_to_rust(op["Cond"]["else_ops"], bind, base_expr,
                                        register_consts, indent + 1, state_expr,
                                        _anchor_ids, _lowering_recipes))
            out.append("%s}" % pad)
        elif "Loop" in op:
            ...
            if (loop.get("reliability") == "Exact" and loop.get("bounded")
                ...
                out.append(ops_to_rust(loop.get("body", []), bind, base_expr,
                                        register_consts, indent + 1, state_expr,
                                        _anchor_ids, _lowering_recipes))
            elif (loop.get("reliability") == "Exact"
                  and loop.get("proof_kind") == "masked_w1c_drain"):
                out.append("%sloop {" % pad)
                out.append(ops_to_rust(loop.get("guard_ops", []), bind, base_expr,
                                        register_consts, indent + 1, state_expr,
                                        _anchor_ids, _lowering_recipes))
                guard_var = loop.get("guard_var", "guard")
                guard_value = expr_to_rust(loop.get("guard_value"))
            ...
            out.append(ops_to_rust(op["Seq"]["ops"], bind, base_expr,
                                    register_consts, indent, state_expr,
                                    _anchor_ids, _lowering_recipes))
        elif "Read" in op:
            o = op["Read"]
            r = rust_mmio_primitive("MmioRead", o)
            a = addr_to_rust(o["addr"], base_expr, register_consts, state_expr)
            var = o["var"]
            wtype = rust_width_type(o["width"])
            ...
            o = op["Write"]
            w = rust_mmio_primitive("MmioWrite", o)
            a = addr_to_rust(o["addr"], base_expr, register_consts, state_expr)
            v = expr_to_rust(o["value"])
            ...
            if recipe.get("kind") == "write_from_read":
                v = expr_to_rust(o.get("transform"))
                out.append("%sunsafe { %s(%s, %s); }" % (pad, w, v, a))
            else:
                t = replace_expr_var(o.get("transform"), o.get("read_var"), "v")
            ...
            o = op["StateRead"]
            index = ("[%s]" % expr_to_rust(o["index"]) if o.get("index") else "")
            out.append("%slet %s = %s.%s%s;" % (pad, o["var"], state_expr, o["field"], index))
        elif "StateWrite" in op:
            o = op["StateWrite"]
            index = ("[%s]" % expr_to_rust(o["index"]) if o.get("index") else "")
            out.append("%s%s.%s%s = %s;" % (pad, state_expr, o["field"], index, expr_to_rust(o["value"])))
        elif "Return" in op:
            out.append("%sreturn %s;" % (pad, expr_to_rust(op["Return"]["value"])))
        elif "Delay" in op:
            out.append("%sreharness_delay_ns(%s);" % (pad, expr_to_rust(op["Delay"]["cycles"])))
    NL = chr(10)
    return NL.join(s for s in out if s)


def rust_local_decls(ops, already_declared, regs, indent=1):
    pad = "    " * indent
    lines = []
    ...
    for v in read_vars:
        lines.append("%slet mut %s: u32 = 0;" % (pad, rust_ident(v)))
    ...
    for v in extra:
        ...
        if re.fullmatch(r"[A-Z][A-Za-z0-9_]*", v):
            continue
        lines.append("%slet %s: u32 = 0;" % (pad, rust_ident(v)))
    NL = chr(10)
...
def generate_rules(formal, device_spec, bind):
    ...
    for fn in device_spec.functions:
        ...
        params = ", ".join("%s: %s" % (rust_ident(p.name), rust_type(p.type, bind)) for p in keep)
        ...
        ret = " -> %s" % rust_type(fn.signature.return_type, bind) if has_return else ""
        L.append("pub fn %s(%s)%s {" % (fn.name, params, ret))
        declared = {rust_ident(p.name) for p in keep} | {"base"}
        decls = rust_local_decls(safe_ops, declared, regs, indent=1)
        if decls:
            L.append(decls)
        L.append("    let base = dev.base;")
        body = ops_to_rust(safe_ops, bind, "base", regs, indent=1,
