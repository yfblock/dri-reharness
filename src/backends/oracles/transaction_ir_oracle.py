...
_KNOWN = {
    ...
    "i2c_transfer": ("TransactionRead", "i2c", "i2c_transfer"),
    "spi_sync": ("TransactionRead", "spi", "spi_sync"),
    "spi_sync_locked": ("TransactionRead", "spi", "spi_sync_locked"),
}
...
def _expected(call: dict) -> dict | None:
    ...
    if transport in {"i2c", "spi"}:
        if len(args) < 2:
            return None
        row["selector"] = _expr(args[1])
        row["payload_kind"] = "Message"
        row["protocol"] = shape
        row["message"] = _expr(args[1])
        if transport == "i2c":
            if len(args) < 3:
                return None
            row["count"] = _expr(args[2])
        return row
    if transport == "regmap":
    ...
    for module in formal.get("modules", []):
        for op in walk_leaf_ops(module.get("ops", [])):
            ...
            if kind == "TransactionUpdate":
                ...
                payload_kind = ("Message" if "Message" in payload
                                else "Buffer" if "Buffer" in payload
                                else "Scalar")
                row["payload_kind"] = payload_kind
                payload_body = payload.get(payload_kind) or {}
                for key in ("value", "buffer", "count", "message"):
                    if key in payload_body:
                        row[key] = payload_body[key]
                if payload_kind == "Message" and body.get("protocol"):
                    row["protocol"] = body["protocol"]
            row["source_offset"] = (body.get("evidence") or {}).get("offset")
            for field in ("target", "selector", "mask", "value",
                          "buffer", "count"):
                if isinstance(row.get(field), dict):
                    row[field] = simplify_expr(row[field])
            rows.append(row)
    return rows


def _formal_transaction_counts(formal: dict) -> tuple[int, int]:
    """Return total and oracle-supported transaction leaf counts."""
    total = 0
    known = 0
    for module in formal.get("modules", []):
        for op in walk_leaf_ops(module.get("ops", [])):
            ...
            if kind is None:
                continue
            total += 1
            if (op[kind].get("evidence") or {}).get("callee") in _KNOWN:
                known += 1
    return total, known


def _verify_transaction_rows(
        formal: dict, expected: list[dict], *, oracle: str) -> dict:
    observed = _observed(formal)
    formal_transactions, known_formal_transactions = (
        _formal_transaction_counts(formal))
    ...
    return {
        ...
        "coverage_complete": (
            not mismatches
            and formal_transactions == known_formal_transactions
            and len(observed_core) == len(expected)),
        "formal_transactions": formal_transactions,
        "known_formal_transactions": known_formal_transactions,
        "expected_transactions": len(expected),
        "observed_transactions": len(observed_core),
        "mismatches": mismatches,
        "scope": ["regmap", "i2c_smbus_scalar", "i2c_message", "spi_message"],
    }


def verify_transaction_sources(formal: dict, sources: list[str]) -> dict:
    """Validate typed transactions against every source in a manifest.

    The observed Formal RIS is already a cross-TU view, so source calls must
    be aggregated before comparison.  This keeps the oracle independent of
    extractor implementation details while avoiding false failures from
    comparing one source's expected rows with all translation units' rows.
    """
    expected = [
        row
        for source in sources
        for row in (_expected(call) for call in _source_calls(source))
        if row is not None
    ]
    return _verify_transaction_rows(
        formal, expected, oracle="multi-source-transaction-shape-v1")


def verify_transaction_source(formal: dict, source: str) -> dict:
    return verify_transaction_sources(formal, [source])
