...
def generate_via_llm(formal, device_spec, bind, *, backend, facts=None,
    ...
    For the harness backend's single-call path (unchunked drivers), the
    fixed scaffold (includes, register defines, backing window, traced
    primitives, raw readers) is emitted mechanically and prepended as
    part 00; the LLM emits only the driver part (part 01).
    """
    ...
    mechanical_scaffold = None
    scaffold_note = ""
    if (backend == "harness"
            and getattr(bind, "primitives", None)):
        try:
            from backends.harness.scaffold import emit_scaffold, summary
            mechanical_scaffold = emit_scaffold(formal, bind)
            scaffold_note = summary(formal, bind)
        except Exception:
            mechanical_scaffold = None
    ...
    def call(mode_extra, module_names=None, function_names=None):
        ...
        prompt = prompt.replace(
            "__SCAFFOLD__",
            scaffold_note or "(see the scaffold rules in this prompt)")
    ...
    if groups is None:
        code, files = call("")
        if mechanical_scaffold is None:
            return GeneratedCode(
                "/* Auto-generated by LLM (reharness) */\n" + code,
                files=files,
            )
        concat = ("/* ---- part 00 of 01 ---- */\n" + mechanical_scaffold
                  + "\n\n/* ---- part 01 of 01 ---- */\n" + code)
        files_out = [
            {"path": f"{backend}.c", "language": "c", "code": ""},
            {"path": "part-00-scaffold.c", "language": "c",
             "code": mechanical_scaffold},
            {"path": "part-01.c", "language": "c", "code": code},
        ]
        return GeneratedCode(
            "/* Auto-generated by LLM (reharness), mechanical scaffold "
            "+ 1 part */\n" + concat,
            files=files_out)

    # chunked path: the scaffold part keeps part index 0 (the repair
    # rounds map module chunk i to part i), so a mechanical scaffold is
    # prepended INSIDE part 00 and the LLM scaffold call emits only the
    # driver-level scaffold (struct, prototypes, stubs, main)
    scaffold_extra = ""
    if mechanical_scaffold is not None:
        scaffold_extra = (
            "\n\nPart 00 already begins with the mechanical scaffold "
            "summarized at __SCAFFOLD__ below (includes, kernel macros, "
            "register offset defines, MMIO backing window, traced "
            "primitives, raw readers). NEVER re-emit any of it — no "
            "includes, no macro or typedef definitions, no register "
            "#defines, no primitive implementations, no trace printfs "
            "(primitives trace themselves). Your scaffold part covers "
            "ONLY: the device private struct, kernel-framework "
            "struct/function stubs, the static prototypes, and the entry "
            "point.")
    n = len(groups) + 1
    scaffold, _ = call(_SCAFFOLD_MODE.format(n=n) + scaffold_extra,
                       module_names=[])
    if mechanical_scaffold is not None:
        scaffold = (mechanical_scaffold + "\n\n" + scaffold)
    parts = [scaffold]
