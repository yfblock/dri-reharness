...
def _differential_cases() -> dict:
    """Case table lives in benchmarks/holdout (data, not core logic)."""
    import json as _json
    path = ROOT / "benchmarks" / "holdout" / "source-differential-cases.json"
    return _json.loads(path.read_text(encoding="utf-8"))["suites"]["gpio_mmio_source"]


def verify_gpio_mmio_source_suite() -> dict:
    from extractor.extractor import ExtractorConfig, extract_ris

    sources = _differential_cases()
    extracted = {
