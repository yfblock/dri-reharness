"""run_backend_pipeline: multi-backend generation + full verification.
...
``gen`` CLI's exit-code logic.  All heavyweight imports stay inside the
function: the pipeline pulls in qa-side oracle modules by path, which is
only valid once sys.path has been prepared.
"""
...
from .compile_repair import _repair_compile
from .paths import (
    _ORACLE_FILES,
    _is_subsequence,
    _pipeline_success,
...
from .receipt_repair import _repair_receipts
