...
def _repository_root() -> Path:
    ...
    for candidate in (current, *current.parents):
        if ((candidate / ".gitmodules").is_file()
            ...
            return candidate
    raise RuntimeError(f"cannot locate reharness repository from {__file__}")


def _repo_roots(linux_root: str | None, build_root: str | None) -> tuple[str, str]:
    repo = _repository_root()
    linux = os.path.abspath(
        linux_root or os.fspath(repo / "vendor/linux"))
    build = build_root or os.environ.get("REHARNESS_KERNEL_BUILD")
    if not build:
        build = os.fspath(repo / "platform/kernel/build")
...
def kbuild_cmd_path(source: str, linux_root: str, build_root: str) -> str | None:
    try:
        relative = os.path.relpath(
            ...
            os.path.realpath(os.path.abspath(linux_root)))
    except ValueError:
        return None
