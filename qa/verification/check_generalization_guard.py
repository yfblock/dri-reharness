...
def _selection_issues(data: dict, manifest_dir: Path) -> list[str]:
    ...
    for index, case in enumerate(cases, 1):
        source = resolve_logical(manifest_dir, case.get("source", ""))
        try:
    ...
    for pool_name, pool in selection.get("pools", {}).items():
        try:
            ...
            continue
        candidates = []
        for source in LINUX_ROOT.glob(glob):
    ...
    for case in cases:
        source = resolve_logical(manifest_dir, case.get("source", ""))
        try:
            actual.append(source.relative_to(LINUX_ROOT).as_posix())
        except ValueError:
            pass
...
def check_guard(holdout_path: str | os.PathLike[str] = DEFAULT_HOLDOUT,
    ...
    actual_linux = _git_rev(LINUX_ROOT)
