...
def _run_make(profile: dict, build: Path, target: str,
    ...
    command = [
        "make", "-s", "-C", str(LINUX_ROOT), f"O={build}",
...
def materialize(holdout_path: Path, recipes_path: Path,
    ...
    for profile_name, case_ids in profile_cases.items():
        ...
        for case_id in sorted(case_ids):
            ...
            source = resolve_logical(holdout_path.parent, case["source"])
            cmd_name = kbuild_cmd_path(
                str(source), str(LINUX_ROOT), str(build))
    ...
    for row in context_rows:
        ...
        context = resolve_compile_context(
            str(source), linux_root=str(LINUX_ROOT),
    ...
    report = {
        ...
        "environment": {
            ...
            "linux_commit": _git_rev(LINUX_ROOT),
        },
        "guard": guard,
...
def main() -> int:
    ...
    parser.add_argument(
        "--database", default=str(
            ARTIFACT_OUTPUT_ROOT /
            "zero-shot-contexts" / "compile_commands.json"))
    parser.add_argument(
        "--output", default=str(
            EXPERIMENT_RESULTS_ROOT / "zero-shot-contexts.json"))
    args = parser.parse_args()
    report = materialize(
        Path(args.holdout).absolute(), Path(args.recipes).absolute(),
