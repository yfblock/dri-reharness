...
def main() -> int:
    ...
    for source in drivers:
        ...
        # A backend counts as compiled only when its C was generated *and* no
        # failure log was written.  Requiring the generated source prevents a
        # skipped/aborted backend (which writes no log either) from being
        # misreported as success.  The baremetal backend compiles to
        # ``/dev/null`` (only callback-oracle builds leave a binary), so the
        # generated source is the stable success signal across all backends.
        def _compiled(kind: str) -> bool:
            return (os.path.isfile(os.path.join(gen_dir, f"{kind}.c"))
        ...
        row = {
            ...
            "backends": {
                ...
                "harness_trace": os.path.exists(os.path.join(ver_dir, "harness.trace.txt")),
