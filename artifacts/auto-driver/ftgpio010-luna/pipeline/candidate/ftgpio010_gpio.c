...
{
	struct gpio_ftgpio010 *priv = gpiochip_get_data(gc);
	void __iomem *base = priv->base;
	u32 val;
					...
					unsigned int offset)
{
	...
	__rh_op_op_61: {
		value = readl(priv->base + GPIO_DATA_IN);
	}
					 ...
					 unsigned int offset, int value)
{
	...
	if (value) {
		...
		__rh_op_op_66: {
			writel(1U << offset, priv->base + GPIO_DATA_SET);
		}
	} else {
		/* REHARNESS_RIS_OP id=op_67 kind=Write status=lowered digest=3834a18c428720bf */
		__rh_op_op_67: {
			writel(1U << offset, priv->base + GPIO_DATA_CLR);
		}
	}
	return 0;
}
						  ...
						  unsigned long *bits)
{
	...
	if (*bits & *mask) {
		...
		__rh_op_op_69: {
			writel(*bits & *mask, priv->base + GPIO_DATA_SET);
		}
	}
	if ((~*bits) & *mask) {
		/* REHARNESS_RIS_OP id=op_70 kind=Write status=lowered digest=5f32f4874eb9aabb */
		__rh_op_op_70: {
			writel((~*bits) & *mask, priv->base + GPIO_DATA_CLR);
		}
	}
	return 0;
}
						   ...
						   unsigned int offset)
{
	...
	__rh_op_op_82: {
		direction = readl(priv->base + GPIO_DIR);
	}
