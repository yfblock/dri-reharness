...
static struct gpio_ftgpio010 *ftgpio_owner;
...
{
	struct gpio_ftgpio010 *priv = ftgpio_owner;
...
{
	struct gpio_ftgpio010 *priv = ftgpio_from_gc(gc);
	void __iomem *base = priv->base;
	u32 val;
...
int ftgpio_gpio_probe__gpio_generic_get(struct gpio_chip *gc,
					unsigned int offset)
{
	struct gpio_ftgpio010 *priv = ftgpio_from_gc(gc);
	void __iomem *base = priv->base;
...
int ftgpio_gpio_probe__gpio_generic_set(struct gpio_chip *gc,
...
int ftgpio_gpio_probe(struct platform_device *pdev)
{
	...
	config.dev = &pdev->dev;
	config.sz = 4;
	config.dat = priv->base + GPIO_DATA_IN;
	config.set = priv->base + GPIO_DATA_SET;
	config.clr = priv->base + GPIO_DATA_CLR;
	config.dirout = priv->base + GPIO_DIR;
	...
	priv->chip.gc.get = ftgpio_gpio_probe__gpio_generic_get;
	priv->chip.gc.get_multiple = ftgpio_gpio_probe__gpio_generic_get_multiple;
	priv->chip.gc.set = ftgpio_gpio_probe__gpio_generic_set;
	priv->chip.gc.set_multiple = ftgpio_gpio_probe__gpio_generic_set_multiple;
	priv->chip.gc.direction_input = ftgpio_gpio_probe__gpio_generic_direction_input;
	priv->chip.gc.direction_output = ftgpio_gpio_probe__gpio_generic_direction_output;
	priv->chip.gc.get_direction = ftgpio_gpio_probe__gpio_generic_get_direction;
