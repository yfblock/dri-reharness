...
static void ftgpio_gpio_irq_handler(struct irq_desc *desc)
{
	...
	girq = irq_desc_get_handler_data(desc);
	if (!girq)
		return;

	gc = container_of(girq, struct gpio_chip, irq);
	...
	while (stat) {
		...
		generic_handle_domain_irq(girq->domain, bit);
		stat &= ~(1U << bit);
	}
}
...
/*
 * This object must remain writable.  gpiolib may install IRQ-chip
 * defaults while registering the gpiochip.
 */
