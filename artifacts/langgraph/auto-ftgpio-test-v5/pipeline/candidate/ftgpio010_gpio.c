...
static struct gpio_ftgpio010 *ftgpio_from_gc(struct gpio_chip *gc)
{
	RH_TRACE_FN("ftgpio_from_gc");
	struct gpio_generic_chip *gchip;

	gchip = container_of(gc, struct gpio_generic_chip, gc);
	return container_of(gchip, struct gpio_ftgpio010, chip);
}

static struct gpio_ftgpio010 *ftgpio_from_irq(struct irq_data *d)
{
						...
						unsigned int offset, int value)
{
	...
	if (value != 0) {
		__rh_op_op_66: {
			writel(1U << offset, g->base + GPIO_DATA_SET);
		}
	}
...
/* REHARNESS_RIS_OP id=op_67 kind=Write status=lowered digest=3834a18c428720bf */
		__rh_op_op_67: {
			writel(1U << offset, g->base + GPIO_DATA_CLR);
		}
	}
	...
	struct gpio_chip *gc, unsigned int offset, int value)
{
	...
	if (value != 0) {
		__rh_op_op_76: {
			writel(mask, g->base + GPIO_DATA_SET);
		}
	}
...
/* REHARNESS_RIS_OP id=op_77 kind=Write status=lowered digest=3834a18c428720bf */
		__rh_op_op_77: {
			writel(mask, g->base + GPIO_DATA_CLR);
		}
	}
...
/* REHARNESS_RIS_OP id=op_45 kind=Read status=lowered digest=e9b515951bfad9ea */
	...
	girq->child_to_parent_hwirq = NULL;
	girq->parent_handler_data = g;
...
/* REHARNESS_RIS_OP id=op_59 kind=Write status=lowered digest=92b66149c6b53a4a */
	...
	/*
	 * Ensure the registered GPIO chip has an explicit firmware node.
	 * This preserves the gpiolib registration route while allowing the
	 * platform registrar to expose the gpio character device.
	 */
	g->chip.gc.fwnode = dev_fwnode(&pdev->dev);

	return devm_gpiochip_add_data(&pdev->dev, &g->chip.gc, g);
}

static const struct of_device_id ftgpio_gpio_of_match[] = {
	{ .compatible = "ft,gpio-ftgpio010" },
	{ }
};
