...
#define GPIO_LINE_DIRECTION_IN  0x1
#define GPIO_LINE_DIRECTION_OUT 0x0
...
static struct driver_priv *priv_from_irq(struct irq_data *d)
{
	RH_TRACE_FN("priv_from_irq");
	return irq_data_get_irq_chip_data(d);
}

static struct driver_priv *priv_from_gc(struct gpio_chip *gc)
{
...
static void ftgpio_gpio_irq_handler(struct irq_desc *desc)
{
	...
	while (stat) {
		...
		generic_handle_domain_irq(priv->gc.irq.domain, bit);
		stat &= ~(1U << bit);
	}
}
...
static int ftgpio_gpio_probe(struct platform_device *pdev)
{
	...
	__rh_op_op_43: {
		priv->gpio_sdata = readl(priv->base + GPIO_DATA_IN);
	}

	/* REHARNESS_RIS_OP id=op_45 kind=Read status=lowered digest=e9b515951bfad9ea */
	__rh_op_op_45: {
		priv->gpio_sdir = readl(priv->base + GPIO_DIR);
	}
	...
	priv->gc.base = -1;
	priv->gc.ngpio = 32;
	priv->gc.get = ftgpio_gpio_probe__gpio_generic_get;
	priv->gc.get_multiple =
		ftgpio_gpio_probe__gpio_generic_get_multiple;
	priv->gc.set = ftgpio_gpio_probe__gpio_generic_set;
	priv->gc.set_multiple =
		ftgpio_gpio_probe__gpio_generic_set_multiple;
	priv->gc.direction_input =
		ftgpio_gpio_probe__gpio_generic_direction_input;
	priv->gc.direction_output =
		ftgpio_gpio_probe__gpio_generic_direction_output;
	priv->gc.get_direction =
		ftgpio_gpio_probe__gpio_generic_get_direction;
	priv->gc.set_config = ftgpio_gpio_set_config;

	priv->irq_chip.name = "ftgpio010";
	priv->irq_chip.irq_ack = ftgpio_gpio_ack_irq;
	priv->irq_chip.irq_mask = ftgpio_gpio_mask_irq;
	priv->irq_chip.irq_unmask = ftgpio_gpio_unmask_irq;
	priv->irq_chip.irq_set_type = ftgpio_gpio_set_irq_type;

	girq = &priv->gc.irq;
	gpio_irq_chip_set_chip(girq, &priv->irq_chip);

	/*
	 * Keep explicit assignments on the registered gpio_irq_chip object.
	 * These supplement the helper call and preserve the callback routes
	 * for registration-AST analysis.
	 */
	girq->chip = &priv->irq_chip;
	girq->parent_handler = ftgpio_gpio_irq_handler;
	priv->gc.irq.parent_handler = ftgpio_gpio_irq_handler;
	girq->num_parents = 1;
	girq->default_type = IRQ_TYPE_NONE;
	girq->handler = handle_bad_irq;

	priv->parent_irq = platform_get_irq(pdev, 0);
	if ((int)priv->parent_irq < 0)
		return (int)priv->parent_irq;

	girq->parents = &priv->parent_irq;

	/* REHARNESS_RIS_OP id=op_56 kind=Write status=lowered digest=7443390abf8fb623 */
	__rh_op_op_56: {
		writel(0, priv->base + GPIO_INT_EN);
	}

	/* REHARNESS_RIS_OP id=op_57 kind=Write status=lowered digest=015d9e37bb50b02a */
	__rh_op_op_57: {
		writel(0, priv->base + GPIO_INT_MASK);
	}

	/* REHARNESS_RIS_OP id=op_58 kind=Write status=lowered digest=153d39994bb1ba88 */
	__rh_op_op_58: {
		writel(0xffffffffU, priv->base + GPIO_INT_CLR);
	}

	/* REHARNESS_RIS_OP id=op_59 kind=Write status=lowered digest=92b66149c6b53a4a */
	__rh_op_op_59: {
		writel(0, priv->base + GPIO_DEBOUNCE_EN);
	}
