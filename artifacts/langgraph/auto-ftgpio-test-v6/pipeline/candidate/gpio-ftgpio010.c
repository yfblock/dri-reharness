...
static inline struct gpio_ftgpio010 *ftgpio_from_desc(struct irq_desc *desc)
{
	RH_TRACE_FN("ftgpio_from_desc");
...
static void ftgpio_gpio_irq_handler(struct irq_desc *desc)
{
	...
	struct gpio_ftgpio010 *g = ftgpio_from_desc(desc);
	void __iomem *base = g->base;
	u32 stat;
...
static int ftgpio_gpio_probe__gpio_generic_get_multiple(struct gpio_chip *gc,
						unsigned long *mask,
						unsigned long *bits)
{
...
static int ftgpio_gpio_probe__gpio_generic_set_multiple(struct gpio_chip *gc,
						unsigned long *mask,
						unsigned long *bits)
{
...
static int ftgpio_gpio_probe__gpio_generic_direction_input(struct gpio_chip *gc,
						unsigned int offset)
{
	RH_TRACE_FN("ftgpio_gpio_probe__gpio_generic_direction_input");
	struct gpio_ftgpio010 *g = ftgpio_from_gc(gc);
	void __iomem *base = g->base;
	u32 shadow = g->gpio_sdir;
	g->gpio_sdir = shadow & ~(1U << offset);
	/* REHARNESS_RIS_OP id=op_74 kind=Write status=lowered digest=b949672343e78260 */
	__rh_op_op_74: {
		writel(g->gpio_sdir, base + GPIO_DIR);
	}
	return 0;
}

static int ftgpio_gpio_probe__gpio_generic_direction_output(struct gpio_chip *gc,
						unsigned int offset, int value)
{
...
static int ftgpio_gpio_probe__gpio_generic_get_direction(struct gpio_chip *gc,
						unsigned int offset)
{
	...
	return (direction & (1U << offset)) ? GPIO_LINE_DIRECTION_OUT : GPIO_LINE_DIRECTION_IN;
}
...
static int ftgpio_gpio_probe(struct platform_device *pdev)
{
	...
	gpio_generic_chip_init(&g->chip, &config);
	g->chip.gc.base = -1;
	g->chip.gc.parent = &pdev->dev;
	g->chip.gc.owner = THIS_MODULE;
	g->chip.gc.get = ftgpio_gpio_probe__gpio_generic_get;
	g->chip.gc.get_multiple = ftgpio_gpio_probe__gpio_generic_get_multiple;
	g->chip.gc.set = ftgpio_gpio_probe__gpio_generic_set;
	g->chip.gc.set_multiple = ftgpio_gpio_probe__gpio_generic_set_multiple;
	g->chip.gc.direction_input = ftgpio_gpio_probe__gpio_generic_direction_input;
	g->chip.gc.direction_output = ftgpio_gpio_probe__gpio_generic_direction_output;
	g->chip.gc.get_direction = ftgpio_gpio_probe__gpio_generic_get_direction;
